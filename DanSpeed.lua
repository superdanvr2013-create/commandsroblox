...
-- ==================== MAGNET TO PLAYER (добавленный скрипт) ====================
local magnetFrame = Instance.new("Frame")
magnetFrame.Size = UDim2.new(0, 300, 0, 100)
magnetFrame.Position = UDim2.new(1, -320, 0, 10)
magnetFrame.BackgroundColor3 = Color3.fromRGB(20, 20, 20)
magnetFrame.BorderSizePixel = 0
magnetFrame.Parent = main

local magnetBtn = Instance.new("TextButton")
magnetBtn.Size = UDim2.new(1, -20, 0, 70)
magnetBtn.Position = UDim2.new(0, 10, 0, 15)
magnetBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
magnetBtn.Text = "Magnet to player"
magnetBtn.TextColor3 = Color3.new(1,1,1)
magnetBtn.TextScaled = true
magnetBtn.Font = Enum.Font.GothamBold
magnetBtn.Parent = magnetFrame

local magnetConnection = nil
local magnetTargetPlayer = nil
local magnetEnabled = false

local function magnetFindNearest()
	local char = speaker.Character
	local root = char and char:FindFirstChild("HumanoidRootPart")
	if not root then return nil end
...
local function toggleMagnet()
	...
	if magnetEnabled then
		magnetTargetPlayer = magnetFindNearest()
		if not magnetTargetPlayer then
			print("Никого рядом нет!")
			magnetEnabled = false
			magnetBtn.BackgroundColor3 = Color3.fromRGB(0, 170, 255)
			magnetBtn.Text = "Magnet to player"
			return
		...
		local hum = speaker.Character and speaker.Character:FindFirstChild("Humanoid")
		if hum then
		...
		print("Magnet включён — быстрое притягивание")
		...
		magnetConnection = RunService.Heartbeat:Connect(function()
			...
			if distance > 1 then
				rootPart.Velocity = direction.Unit * math.min(distance * 9, 35)
			else
