...
local function updateDetachedControl()
    ...
    if velocityCtrl then velocityCtrl.Velocity = moveDirection * 50 end
    if gyro then
    ...
    if xrayActive then
        ...
        for _, part in pairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" and part.Name ~= "LevitatePart" then
                local distance = (part.Position - (speaker.Character and speaker.Character:FindFirstChild("HumanoidRootPart") and speaker.Character.HumanoidRootPart.Position or Vector3.new())).Magnitude
...
local function createTeleportButton()
    ...
    targetText.Text = "🎯 Ближайший: " .. (findNearestPlayer() and findNearestPlayer().Name or "нет игроков")
    ...
    teleportButton.MouseButton1Click:Connect(teleportToNearest)
end
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == Enum.KeyCode.Z and isSomeoneActive then
        teleportToNearest()
    end
...
speaker.CharacterAdded:Connect(function()
    ...
    if animationsActive then toggleAnimations(true) end
    if detachLowerTorsoActive then
        detachLowerTorsoActive = false
        reattachLowerTorso()
    end
