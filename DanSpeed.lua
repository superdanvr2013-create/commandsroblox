-- === EliteX Lite — Full Original + Instant Purchase + Magnet + Auto Bat ===
...
local autoBatEnabled = false          -- НОВАЯ ПЕРЕМЕННАЯ
local autoBatConnection = nil         -- НОВАЯ ПЕРЕМЕННАЯ
...
local function teleportToNearest()
    ...
    if targetPlayer and targetPlayer.Character then
        ...
        if targetRoot and playerRoot then
            ...
            -- ... (beam эффект остаётся без изменений)
    ...
    if xrayActive then
        ...
        for _, part in pairs(workspace:GetDescendants()) do
            if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" and part.Name ~= "LevitatePart" then
                if isNearPlayerButNotUnder(part) then
...
local function updateXray()
    ...
    -- (полный код updateXray остаётся как был)
    local char = speaker.Character
    local root = char and char:FindFirstChild("HumanoidRootPart")
    ...
    for _, part in pairs(workspace:GetDescendants()) do
        if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" and part.Name ~= "LevitatePart" then
            ...
            elseif originalTransparencies[part] then
                ...
                for i, p in ipairs(xrayParts) do
                    if p == part then
    ...
    for _, other in ipairs(Players:GetPlayers()) do
        if other ~= speaker and other.Character then
            ...
            if otherRoot then
                ...
                if dist < minDist and dist > 6 then
                    minDist = dist
                    nearest = other
                end
            end
...
local function toggleMagnet()
    ...
    if magnetEnabled then
        ...
        magnetConnection = RunService.Heartbeat:Connect(function()
            if not magnetEnabled or not magnetTargetPlayer or not magnetTargetPlayer.Character then return end
            ...
            if distance > 1 then
                root.Velocity = direction.Unit * math.min(distance * 9, 60)
            else
    ...
    if autoBatEnabled then
        ...
        autoBatConnection = RunService.Heartbeat:Connect(function()
            ...
            for _, tool in ipairs(char:GetChildren()) do
                if tool:IsA("Tool") and (tool.Name == "Bat" or string.find(tool.Name:lower(), "bat")) then
                    tool:Activate()
                    task.wait(0.08)
                    tool:Deactivate()
                    break
...
local function applyBoost()
    ...
    if hum then
        hum.WalkSpeed = boostActive and 30 or originalSpeed
        hum.JumpPower = boostActive and 10 or originalJump
    end
end
...
Frame.Size = UDim2.new(0, 250, 0, 620)  -- увеличено под новую кнопку
Frame.Active = true
...
title.Parent = Frame
title.Text = "ELITEX — Lite + Purchase + Magnet + Auto Bat"
...
local magnetBtn     = createBtn("MagnetBtn",     "🧲 MAGNET TO PLAYER: OFF", 285, Color3.fromRGB(0, 170, 255))
local autoBatBtn    = createBtn("AutoBatBtn",    "🦇 AUTO BAT: OFF",        325, Color3.fromRGB(0, 170, 255))  -- НОВАЯ КНОПКА
local espBtn        = createBtn("EspBtn",        "ESP: OFF",                365, Color3.fromRGB(80, 80, 80))
local levitationBtn = createBtn("LevitationBtn", "ЛЕВИТАЦИЯ: OFF",          405, Color3.fromRGB(120, 40, 200))
local anchorBtn     = createBtn("AnchorBtn",     "ANCHORED: OFF",           445, Color3.fromRGB(40, 40, 45))
local kickBtn       = createBtn("KickBtn",       "KICK",                    485, Color3.fromRGB(255, 50, 50))
...
magnetBtn.MouseButton1Click:Connect(function()
    ...
    magnetBtn.BackgroundColor3 = magnetEnabled and Color3.fromRGB(255, 80, 80) or Color3.fromRGB(0, 170, 255)
end)

-- === НОВАЯ КНОПКА AUTO BAT ===
autoBatBtn.MouseButton1Click:Connect(function()
    toggleAutoBat()
    autoBatBtn.Text = autoBatEnabled and "🦇 AUTO BAT: ON" or "🦇 AUTO BAT: OFF"
    autoBatBtn.BackgroundColor3 = autoBatEnabled and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(0, 170, 255)
end)
...
-- Обработчики клавиш (без изменений)
...
-- ProximityPrompt для Instant Purchase
...
speaker.CharacterAdded:Connect(function()
    ...
    if autoBatEnabled then
        toggleAutoBat() -- выключаем
        task.wait(0.3)
        toggleAutoBat() -- включаем заново
    end
end)
...
closeBtn.MouseButton1Click:Connect(function()
    ...
    if autoBatEnabled then toggleAutoBat() end
    if magnetConnection then magnetConnection:Disconnect() end
